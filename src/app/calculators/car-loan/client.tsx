...
const formSchema = z.object({
  carPrice: z.coerce.number().positive("Car price must be a positive number."),
  ...
  if (data.downPayment + data.tradeInValue >= data.carPrice) {
    ctx.addIssue({
      ...
      path: ["downPayment"],
      message: "Down payment plus trade-in must be less than the car price.",
    });
  }
});
...
function CarLoanCalculatorForm({
  setResult,
}: {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    const principal =
      values.carPrice - values.downPayment - values.tradeInValue;
    if (principal <= 0) {
      const resultData = {
        monthlyPayment: 0,
        totalInterest: 0,
        ...
        chartData: [
          { name: "principal" as const, value: principal > 0 ? principal : 0 },
          { name: "interest" as const, value: 0 },
        ],
    ...
    <div className="space-y-4">
      {result.loanAmount <= 0 ? (
        <div className="text-center">
          <p className="text-2xl font-bold font-headline">No Loan Needed</p>
          <p className="text-muted-foreground">
            Your down payment and trade-in cover the car price.
          </p>
        </div>
      ) : (
        <>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input type="number" step="any" placeholder="e.g., 25000" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input type="number" step="any" placeholder="e.g., 0" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
...
export function CarLoanCalculatorPageClient({
  calculator,
  faqSchema,
}: {
  ...
  return (
    <CalculatorPage
      ...
      faqSchema={faqSchema}
    >
      <CarLoanCalculatorForm setResult={setResult} />
    </CalculatorPage>
  );
}
